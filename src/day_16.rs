use itertools::Itertools;

#[derive(Default)]
struct Decoder {
    bits: Vec<char>,
    offset: usize,
    versions: Vec<usize>,
    values: Vec<usize>,
}

trait Converter {
    fn to_decimal(&self) -> usize;
}

impl Converter for Vec<char> {
    fn to_decimal(&self) -> usize {
        let b = self.iter().collect::<String>();
        usize::from_str_radix(&b, 2).unwrap()
    }
}

impl Decoder {
    fn new(data: &str) -> Self {
        let bits = data
            .chars()
            .into_iter()
            .map(Self::char_to_binary)
            .map(|s| s.chars())
            .flatten()
            .collect_vec();

        Self::new_from_bits(bits)
    }

    fn new_from_bits(bits: Vec<char>) -> Self {
        Decoder {
            bits,
            ..Default::default()
        }
    }

    fn new_from_offset(&mut self) -> Self {
        let bits = self.bits[self.offset..].to_owned();
        Self::new_from_bits(bits)
    }

    fn decode(&mut self) {
        let pkt_ver = self.take_next(3).to_decimal();
        self.versions.push(pkt_ver);
        let pkt_type = self.take_next(3).to_decimal();
        match pkt_type {
            4 => {
                let val = self.literal_pkt();
                self.values.push(val);
            }
            _ => {
                self.operator_pkt(pkt_type);
            }
        };
    }

    fn calc_values(&mut self, pkt_type: usize) -> usize {
        match pkt_type {
            0 => self.values.iter().sum::<usize>(),
            1 => self.values.iter().product::<usize>(),
            2 => *self.values.iter().min().unwrap(),
            3 => *self.values.iter().max().unwrap(),
            5 => {
                if self.values[0] > self.values[1] {
                    1
                } else {
                    0
                }
            }
            6 => {
                if self.values[0] < self.values[1] {
                    1
                } else {
                    0
                }
            }
            7 => {
                if self.values[0] == self.values[1] {
                    1
                } else {
                    0
                }
            }
            _ => unreachable!(),
        }
    }

    fn operator_pkt(&mut self, pkt_type: usize) {
        let length = self.take_next(1).to_decimal();
        match length {
            0 => {
                let sub_pkt_len = self.take_next(15).to_decimal();
                let new_pkt = self.take_next(sub_pkt_len);
                let mut new_decoder = Decoder::new_from_bits(new_pkt);
                while new_decoder.offset < sub_pkt_len {
                    new_decoder.decode();
                }
                self.versions.push(new_decoder.sum_versions());
                self.values.push(new_decoder.calc_values(pkt_type));
            }
            1 => {
                let sub_pkt_num = self.take_next(11).to_decimal();
                let mut new_decoder = self.new_from_offset();
                for _ in 0..sub_pkt_num {
                    new_decoder.decode();
                }
                self.versions.push(new_decoder.sum_versions());
                self.values.push(new_decoder.calc_values(pkt_type));
                self.offset += new_decoder.offset;
            }
            _ => unreachable!(),
        }
    }

    fn literal_pkt(&mut self) -> usize {
        let mut vals = vec![];
        loop {
            let next = self.take_next(5);
            vals.push(next[1..next.len()].to_owned());
            if next[0] == '0' {
                break;
            }
        }
        vals.into_iter().flatten().collect_vec().to_decimal()
    }

    fn take_next(&mut self, next: usize) -> Vec<char> {
        let from = self.offset;
        self.offset += next;
        self.bits[from..self.offset].to_owned()
    }

    fn sum_versions(&self) -> usize {
        self.versions.iter().sum()
    }

    fn char_to_binary(c: char) -> &'static str {
        match c {
            '0' => "0000",
            '1' => "0001",
            '2' => "0010",
            '3' => "0011",
            '4' => "0100",
            '5' => "0101",
            '6' => "0110",
            '7' => "0111",
            '8' => "1000",
            '9' => "1001",
            'A' => "1010",
            'B' => "1011",
            'C' => "1100",
            'D' => "1101",
            'E' => "1110",
            'F' => "1111",
            _ => "",
        }
    }
}

#[cfg(test)]
mod tests {
    use crate::day_16::Decoder;

    #[test]
    fn should_decode_test_data() {
        let mut dec = Decoder::new("");
        dec.decode();
        assert_eq!(906, dec.sum_versions())
    }

    #[test]
    fn should_decode_part_2() {
        let mut dec = Decoder::new("C200B40A82");
        dec.decode();
        println!("{:?}", dec.values);
        assert_eq!(3, dec.values[0]);
    }
    #[test]
    fn should_decode_part_2_1() {
        let mut dec = Decoder::new("04005AC33890");
        dec.decode();
        assert_eq!(54, dec.values[0]);
    }
    #[test]
    fn should_decode_part_2_2() {
        let mut dec = Decoder::new("880086C3E88112");
        dec.decode();
        assert_eq!(7, dec.values[0]);
    }
    #[test]
    fn should_decode_part_2_3() {
        let mut dec = Decoder::new("CE00C43D881120");
        dec.decode();
        assert_eq!(9, dec.values[0]);
    }
    #[test]
    fn should_decode_part_2_4() {
        let mut dec = Decoder::new("D8005AC2A8F0");
        dec.decode();
        assert_eq!(1, dec.values[0]);
    }
    #[test]
    fn should_decode_part_2_5() {
        let mut dec = Decoder::new("F600BC2D8F");
        dec.decode();
        assert_eq!(0, dec.values[0]);
    }
    #[test]
    fn should_decode_part_2_6() {
        let mut dec = Decoder::new("9C005AC2F8F0");
        dec.decode();
        assert_eq!(0, dec.values[0]);
    }

    #[test]
    fn should_decode_part_2_7() {
        let mut dec = Decoder::new("9C0141080250320F1802104A08");
        dec.decode();
        println!("{:?}", dec.values);
        assert_eq!(1, dec.values[0]);
    }

    #[test]
    fn should_decode_part_2_8() {
        let mut dec = Decoder::new("");
        dec.decode();
        assert_eq!(819324480368, dec.values[0]);
    }
}
